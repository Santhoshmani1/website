...
const Header = () => {
  ...
  return (
    <header
      ref={headerRef}
      className={`fixed z-20 w-full p-1 text-xs border-b rounded-sm shadow-md md:px-32 ${theme === "light"
          ? "bg-sky-50 border-b-gray-200 text-black"
          : "bg-slate-900 border-b-slate-600 text-white"
        }`}
    >
      <div className="container px-4 py-2 mx-auto lg:px-6 md:flex md:items-center md:justify-between">
        <div className="flex items-center justify-between">
          ...
          <button
            className="material-icons sm:hidden" title="menu"
            onClick={() => {
              setIsOpen(!isOpen);
        ...
        <ul className="flex flex-col items-center justify-center md:flex-row md:space-x-10 md:space-y-0">
            {navigationListItems.map((item, index) => (
              <li key={index}>
                <Link
                  to={item.path}
                  className="text-lg leading-loose text-left transition duration-300 hover:text-blue-400"
                  onClick={() => {
                    setIsOpen(false);
                    setMenuIcon("menu");
                  }}
